...
var _imageKeeperJs = require("./imageKeeper.js");
const ws = new WebSocket("ws://localhost:3001");
const feedDiv = document.getElementById("feed");
function connectWebSocket({ allowNSFW = false } = {}) {
    const ws = new WebSocket("ws://localhost:3001");
    const feedDiv = document.getElementById("feed");
    (0, _imageKeeperJs.initializeImageKeeper)({
        allowNSFW
    });
    ws.onmessage = (event)=>{
        ...
        images.forEach((image)=>{
            (0, _imageKeeperJs.addImage)(image, allowNSFW);
        });
    };
    ws.onclose = ()=>{
...
},{"./imageKeeper.js":"bcXXx","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"bcXXx":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "initializeImageKeeper", ()=>initializeImageKeeper);
...
function initializeImageKeeper({ allowNSFW: nsfw = false } = {}) {
    allowNSFW = nsfw;
...
function addImage(imageObject) {
    ...
    const isNSFW = classification.some((c)=>(c.className === "Porn" || c.className === "Hentai") && c.probability > NSFW_THRESHOLD);
    if (!allowNSFW && isNSFW) {
        console.warn("NSFW content filtered");
        return null;
    }
    if (images.length >= MAX_IMAGES) images.shift(); // Remove oldest image
...
const drawFrame = ()=>{
    ...
    if (!isMouseDown) {
        ...
        // Handle tick counter
        if (ticksTillNextImage > 0) ticksTillNextImage--;
        // Main drawing logic
        if (blackPause === 0 && images.length > 0) {
            ...
            if (currentState === States.EXPLORE) {
                ...
                ctx.drawImage(currentImage.element, x, y, sourceWidth, sourceHeight, 0, 0, viewportWidth, viewportHeight);
                if (Math.random() < 0.2) currentState = States.NORMAL;
                blackPause = 1;
            }
            if (currentState === States.NORMAL) {
                if (!currentImage || Math.random() < 0.7) currentImage = images[Math.floor(Math.random() * images.length)];
                ctx.drawImage(currentImage.element, 0, 0, viewportWidth, viewportHeight);
                if (Math.random() < 0.15) currentState = States.EXPLORE;
                // Set black pause duration based on number of images
                if (images.length < 5) blackPause = Math.floor(Math.random() * (MAX_IMAGES - images.length) * 3);
                else if (images.length < 10) blackPause = Math.floor(Math.random() * 4);
                else blackPause = Math.floor(Math.random() * 2);
                isClear = false;
            }
            // Store the last successfully drawn image
            if (currentImage && !isClear) lastDisplayedImage = currentImage;
        } else {
            ...
            if (blackPause > 0) blackPause--;
        }
    } else if (lastDisplayedImage) // When mouse is down, keep showing the last displayed image
    ctx.drawImage(lastDisplayedImage.element, 0, 0, viewportWidth, viewportHeight);
...
},{}]},["farZc","8lqZg"], "8lqZg", "parcelRequiree8ef")
